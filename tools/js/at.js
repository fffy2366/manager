  ...
  var at = (function () {
    ...
    it.check=function () {
      ...
      if((key = value.match(reg)) && (key=key[0]) && /^@[a-zA-Z0-9\u4e00-\u9fa5_]+$/.test(key)) {
        ...
        var tmpUrl = siteUrl;
        if (siteUrl.lastIndexOf('/action.php') != -1) {
        	tmpUrl = siteUrl.substring(0, siteUrl.lastIndexOf('/action.php'));
        } else if (siteUrl.lastIndexOf('/?anu=') != -1) {
        	tmpUrl = siteUrl.substring(0, siteUrl.lastIndexOf('/?anu='));
        }
        siteUrl = tmpUrl;
        doRequest(siteUrl+'/action.php?anu=at/suggestion&k=' + encodeURIComponent(key), function(json){
          it.initTip(json,key);
        },select.hidden);
        return;
      }
